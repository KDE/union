...
class TestAlignmentPropertyGroup : public QObject
{
    Q_OBJECT
private Q_SLOTS:
    void testNull()
    {
        auto property = std::make_unique<AlignmentPropertyGroup>();
    ...
    void testEmpty()
    {
        auto property = AlignmentPropertyGroup::empty();
    ...
    {
        auto source = std::make_unique<AlignmentPropertyGroup>();
        auto destination = std::make_unique<AlignmentPropertyGroup>();

        QVERIFY(!source->hasAnyValue());
        QVERIFY(!destination->hasAnyValue());

        // Calling resolve on empty source and destination should have no effect.
        AlignmentPropertyGroup::resolveProperties(source.get(), destination.get());
    ...
    void testDataStream()
    {
        auto group = testAlignmentPropertyGroupInstance();
        ...
        auto readGroup = std::make_unique<AlignmentPropertyGroup>();
...
QTEST_MAIN(TestAlignmentPropertyGroup)

#include "TestAlignmentPropertyGroup.moc"