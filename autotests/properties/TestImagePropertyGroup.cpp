...
class TestImagePropertyGroup : public QObject
{
    Q_OBJECT
private Q_SLOTS:
    void testNull()
    {
        auto property = std::make_unique<ImagePropertyGroup>();
    ...
    void testEmpty()
    {
        auto property = ImagePropertyGroup::empty();
    ...
    {
        auto source = std::make_unique<ImagePropertyGroup>();
        auto destination = std::make_unique<ImagePropertyGroup>();

        QVERIFY(!source->hasAnyValue());
        QVERIFY(!destination->hasAnyValue());

        // Calling resolve on empty source and destination should have no effect.
        ImagePropertyGroup::resolveProperties(source.get(), destination.get());
    ...
    void testDataStream()
    {
        auto group = testImagePropertyGroupInstance();
        ...
        auto readGroup = std::make_unique<ImagePropertyGroup>();
...
QTEST_MAIN(TestImagePropertyGroup)

#include "TestImagePropertyGroup.moc"