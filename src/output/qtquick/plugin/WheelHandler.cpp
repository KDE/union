...
// This is a code freeze from Kirigami's WheelHandler until Flickable is made more configurable
// Please see this MR: plasma/union!117

UnionWheelEvent::UnionWheelEvent(QObject *parent)
    : QObject(parent)
{
}

UnionWheelEvent::~UnionWheelEvent()
{
}

void UnionWheelEvent::initializeFromEvent(QWheelEvent *event)
{
...
qreal UnionWheelEvent::x() const
{
    return m_x;
}

qreal UnionWheelEvent::y() const
{
    return m_y;
}

QPointF UnionWheelEvent::angleDelta() const
{
    return m_angleDelta;
}

QPointF UnionWheelEvent::pixelDelta() const
{
    return m_pixelDelta;
}

int UnionWheelEvent::buttons() const
{
    return m_buttons;
}

int UnionWheelEvent::modifiers() const
{
    return m_modifiers;
}

bool UnionWheelEvent::inverted() const
{
    return m_inverted;
}

bool UnionWheelEvent::isAccepted()
{
    return m_accepted;
}

void UnionWheelEvent::setAccepted(bool accepted)
{
    m_accepted = accepted;
}
...
void WheelHandler::_k_rebindScrollBars()
{
    ...
    if (m_flickable) {
        ...
        // parent is another Flickable that already has a WheelHandler.
...
bool WheelHandler::scrollFlickable(QPointF pixelDelta, QPointF angleDelta, Qt::KeyboardModifiers modifiers)
{
    ...
    auto setPosition = [this, devicePixelRatio, refreshRate](qreal oldPos, qreal newPos, qreal stepSize, const char *property, QPropertyAnimation &animation) {
        ...
        if (!Union::StyleRegistry::instance()->platform()->smoothScroll() || refreshRate <= 0) {
        ...
        // TODO: !!! Replace this with a proper interface for acquiring long duration
        int longDuration = 200;

        qreal absPixelDelta = std::abs(newPos - oldPos);
        int duration = absPixelDelta * devicePixelRatio > 2 //
            ? std::max(qCeil(1000.0 / 60.0 * 3), std::min(qRound(absPixelDelta * longDuration / stepSize), longDuration))
            : 0;
    ...
    case QEvent::Wheel: {
        ...
        if (wheelEvent->pixelDelta() == wheelEvent->angleDelta()) {
            ...
            QWheelEvent newWheelEvent(wheelEvent->position(),
                                      ...
                                      wheelEvent->inverted(),
                                      wheelEvent->source());
            m_unionWheelEvent.initializeFromEvent(&newWheelEvent);
        } else {
            m_unionWheelEvent.initializeFromEvent(wheelEvent);
        }

        Q_EMIT wheel(&m_unionWheelEvent);
        ...
        if (m_unionWheelEvent.isAccepted()) {
            return true;
        }
        ...
        if (m_scrollFlickableTarget || (contentHeight <= pageHeight && contentWidth <= pageWidth)) {
            ...
            QPointF pixelDelta = m_unionWheelEvent.angleDelta().isNull() ? m_unionWheelEvent.pixelDelta() : QPoint(0, 0);
            scrolled = scrollFlickable(pixelDelta, m_unionWheelEvent.angleDelta(), Qt::KeyboardModifiers(m_unionWheelEvent.modifiers()));
        }
