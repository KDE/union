...
class UNIONPLATFORM_NO_EXPORT KColorSchemeProvider::Private
{
public:
    Union::LruCache<qsizetype, Union::ColorProvider::Rgba> cache;
    KSharedConfigPtr colorConfig;
};
...
{
    if (arguments.size() != 4) {
        qCWarning(UNION_PLATFORM) << "Invalid arguments for KColorScheme color provider, expected 4 got" << arguments.size();
        return std::nullopt;
    }

    auto argumentsHash = qHash(arguments, QHashSeed::globalSeed());
    if (d->cache.contains(argumentsHash)) {
        return d->cache.value(argumentsHash).value();
    }

    auto group = colorGroupFromString(arguments.at(0));
    if (!group) {
        qCWarning(UNION_PLATFORM) << "Invalid arguments for KColorScheme color provider, group" << arguments.at(0) << "does not exist";
        return std::nullopt;
    }

    auto colorSet = colorSetFromString(arguments.at(1));
    if (!colorSet) {
        qCWarning(UNION_PLATFORM) << "Invalid arguments for KColorScheme color provider, color set" << arguments.at(1) << "does not exist";
        return std::nullopt;
    }

    KColorScheme colorScheme(group.value(), colorSet.value(), d->colorConfig);
    ...
    if (roleType.compare(u"background", Qt::CaseInsensitive) == 0) {
        ...
        if (role) {
            ...
            qCWarning(UNION_PLATFORM) << "Invalid arguments for KColorScheme color provider, background role" << arguments.at(3) << "does not exist";
        }
        ...
        if (role) {
            ...
            qCWarning(UNION_PLATFORM) << "Invalid arguments for KColorScheme color provider, foreground role" << arguments.at(3) << "does not exist";
        }
        ...
        if (arguments.at(3) == u"frame") {
            ...
            if (role) {
                ...
                qCWarning(UNION_PLATFORM) << "Invalid arguments for KColorScheme color provider, decoration role" << arguments.at(3) << "does not exist";
            }
        }
    } else {
        qCWarning(UNION_PLATFORM) << "Invalid arguments for KColorScheme color provider, role type" << roleType << "does not exist";
    }

    if (value) {
        d->cache.insert(argumentsHash, value.value());
    }
